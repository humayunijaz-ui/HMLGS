...
-- ------------------------------------------------------------
-- admission_quotas (Open Merit / Self-Finance / Reserved / etc.)
-- ------------------------------------------------------------
...
CREATE TABLE hostel_applications (
    ...
    admission_quota_id INT DEFAULT NULL,
    degree VARCHAR(50) DEFAULT NULL,
    ...
    CONSTRAINT fk_app_quota FOREIGN KEY (admission_quota_id) REFERENCES admission_quotas(id),
...
-- ------------------------------------------------------------
-- quota_seat_matrix (seat limits: Department > Program > Gender > Quota, per session)
-- ------------------------------------------------------------
CREATE TABLE quota_seat_matrix (
    id INT AUTO_INCREMENT PRIMARY KEY,
    hostel_session_id INT NOT NULL,
    department_id INT NOT NULL,
    program_id INT NOT NULL,
